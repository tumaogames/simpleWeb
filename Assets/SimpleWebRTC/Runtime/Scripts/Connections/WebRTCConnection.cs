...
namespace SimpleWebRTC {
    public class WebRTCConnection : MonoBehaviour {
        ...
        [SerializeField] private Unity.WebRTC.RenderTextureDepth RTDepth = Unity.WebRTC.RenderTextureDepth.Depth24;
        ...
        private void Awake() {
            ...
            if (UseImmersiveSetup) {
                if (IsVideoAudioSender) {
                    int rtDepthBits = (int)RTDepth;
                    cubemapLeftEye = new RenderTexture(OneEyeRenderSide, OneEyeRenderSide, rtDepthBits, RenderTextureFormat.BGRA32);
                    cubemapLeftEye.dimension = TextureDimension.Cube;
                    cubemapLeftEye.hideFlags = HideFlags.HideAndDontSave;
                    cubemapLeftEye.Create();

                    if (RenderStereo) {
                        cubemapRightEye = new RenderTexture(OneEyeRenderSide, OneEyeRenderSide, rtDepthBits, RenderTextureFormat.BGRA32);
                        ...
                        videoEquirect = new RenderTexture(OneEyeRenderSide, OneEyeRenderSide * 2, rtDepthBits, RenderTextureFormat.BGRA32);
                    } else {
                        videoEquirect = new RenderTexture(OneEyeRenderSide, OneEyeRenderSide, rtDepthBits, RenderTextureFormat.BGRA32);
                    }
